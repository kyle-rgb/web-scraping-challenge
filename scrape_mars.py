...
def scrape():
    final_dict = {}
    #Mars News
    mars_news_url = "https://mars.nasa.gov/news/8744/nasa-engineers-checking-insights-weather-sensors/"
    ...
    facts_response = requests.get("https://space-facts.com/mars/")
    table = pd.read_html("https://space-facts.com/mars/")
    table[0]
    table = table[0]
    table.rename({0: "Facts", 1:"Mars"}, axis=1, inplace=True)
    table.set_index("Facts", inplace=True)
    table.to_html("table.html")
    
    final_dict["Planet_Facts"] = {}
    final_dict["Planet_Facts"]["table"] = table.to_dict()
    

    # Hemisphere Requests
    response_4 = requests.get("https://astrogeology.usgs.gov/search/results?q=hemisphere+enhanced&k1=target&v1=Mars")
    soupabc = bs4.BeautifulSoup(response_4.text, "lxml")
    soupabc.select("h3")

    hemisphere_list = [{"img_url": "https://astropedia.astrogeology.usgs.gov/download/Mars/Viking/cerberus_enhanced.tif/full.jpg"},
                         {"img_url": "https://astropedia.astrogeology.usgs.gov/download/Mars/Viking/schiaparelli_enhanced.tif/full.jpg"},
                         {"img_url": "https://astropedia.astrogeology.usgs.gov/download/Mars/Viking/syrtis_major_enhanced.tif/full.jpg"},
                         {"img_url": "https://astropedia.astrogeology.usgs.gov/download/Mars/Viking/valles_marineris_enhanced.tif/full.jpg"}]

    for name, dictionary in zip(soupabc.select('h3'), hemisphere_list):
        dictionary["title"] = name.text

    final_dict["Hemispheres"] = hemisphere_list
